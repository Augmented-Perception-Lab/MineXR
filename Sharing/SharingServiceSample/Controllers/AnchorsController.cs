...
namespace SharingService.Controllers
{
    ...
    public class AnchorsController : ControllerBase
    {
        ...
        // DELETE api/anchors/{anchorNumber}
        [HttpDelete("{anchorNumber}")]
        public async Task<ActionResult<string>> DeleteAsync(long anchorNumber)
        {
            // Get the key if present
            try
            {
                return await this.anchorKeyCache.DeleteAnchorAsync(anchorNumber);
            }
            catch (KeyNotFoundException)
            {
                return this.NotFound();
