...
namespace SharingService.Data
{
    internal class MemoryAnchorCache : IAnchorKeyCache
    {
        ...
        /// <summary>
        /// Deletes the anchor asynchronously.
        /// </summary>
        /// <param name="anchorId">The anchor ID.</param>
        /// <returns>The anchor key of the deleted anchor.</returns>
        public async Task<string> DeleteAnchorAsync(long anchorId)
        {

            if (this.memoryCache.TryGetValue(anchorId, out string anchorKey))
            {
                this.memoryCache.Remove(anchorKey);

                return anchorKey;
            }

            return $"The {nameof(anchorId)} {anchorId} not found";
        }
    }
}
