...
class VirtualObjectInteraction: NSObject, UIGestureRecognizerDelegate {
    ...
    func didTap(_ gesture: UITapGestureRecognizer) {
        ...
        /// For phone position only mode for placement
        /// If object selection mode is on, just select the object, never set down
        if viewController.isRaycast {
            if let tappedObject = sceneView.virtualObject(at: touchLocation) {
                
                // If an object exists at the tap location, select it.
                selectedObject = tappedObject
                
                // TODO: Add some visual indicator to show which object was selected
                let material = selectedObject?.childNodes.first?.geometry!.firstMaterial!
                ...
                print("raycast mode no object selected")
            }
        } else {
        ...
        /// For raycast / phone position dual mode
        /*
        if let tappedObject = sceneView.virtualObject(at: touchLocation) {
    ...
    */
    ...
    func translate(_ object: VirtualObject, basedOn screenPos: CGPoint) {
        ...
        if let query = sceneView.raycastQuery(from: screenPos, allowing: .existingPlaneInfinite, alignment: object.allowedAlignment) {
    ...
    func setDown(_ object: VirtualObject, basedOn screenPos: CGPoint) {
        ...
        if (!object.isInScene) {
            viewController.placeObjectInScene(virtualObject: object)
        ...
        if let query = object.raycastQuery,
//        if let query = sceneView.raycastQuery(from: screenPos, allowing: .existingPlaneInfinite, alignment: object.allowedAlignment),
            ...
            viewController.updateQueue.async {
                _ = self.sceneView.addOrUpdateAnchor(for: object)
                self.viewController.addOrUpdateCloudAnchor(for: object)
            }
        }
    }
}
