...
struct PhotoPicker: UIViewControllerRepresentable {
    ...
    func makeUIViewController(context: UIViewControllerRepresentableContext<PhotoPicker>) -> PHPickerViewController {
        ...
        // Allow multiple selections. Setting the value to 0 sets the selection limit to the maximum that the system supports.
...
class Coordinator: NSObject, UINavigationControllerDelegate, PHPickerViewControllerDelegate {
    ...
    func picker(_ picker: PHPickerViewController, didFinishPicking results: [PHPickerResult]) {
        ...
        print("Saving \(results.count) photos")
        
        // Dismiss the presented picker.
        self.parent.dismiss()

        // Save all the selected photos to the data model
                    ...
                    if let savedUrl = FileManager.default.copyItemToDocumentDirectory(from: url) {
                        ...
                        Task { @MainActor [dataModel = self.parent.dataModel] in
                            withAnimation {
                                ...
                                print("Screenshot saved to: \(savedUrl.absoluteString)")
                                
                                // Upload screenshot to Firestore
                                self.parent.uploadScreenshot(savedUrl)
                            }
                        }
                    }
                }
